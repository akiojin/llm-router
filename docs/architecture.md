...
- エンドポイント（xLLM、Ollama、vLLM、LM Studio、llama.cpp 等）を**ブラックボックス**として扱う
...
正常に動作し、TPSベースの負荷分散に参加します。

## 3. 負荷分散戦略: TPS優先

llmlbは**TPS（Tokens Per Second）優先**方式を採用します。

```text
エンドポイントA (TPS: 80 tok/s) ← 優先
エンドポイントB (TPS: 50 tok/s)
エンドポイントC (TPS: 未計測=0) ← 最低優先
```
...
| 項目 | 説明 |
|------|------|
| 計測タイミング | 推論リクエスト成功時（出力トークン数÷処理時間） |
| 計算方式 | 指数移動平均（EMA、α=0.2） |
| 初期値 | 0.0（最低優先度、TPS未計測） |
| オフライン時 | 0.0にリセット |
| 補助指標 | レイテンシはSQLiteに永続化（ダッシュボード表示・デバッグ用） |

### タイブレーク

複数エンドポイントが同一TPSの場合、ラウンドロビンでタイブレークします。

### ゲートウェイ設計との整合性

TPS計測は推論リクエストの出力トークン数とレスポンス時間を利用するため、
