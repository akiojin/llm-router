...
コーディネーターが公式にサポートするモデルは次の5件に限定する。
...
- エージェント登録完了後、対応モデル5件をまとめて自動ダウンロードし常駐させる（手動配布は不要）。
...
### モデル起動完了までの受付制御
- 対応モデル5件がすべて起動・常駐完了するまではエージェントは `initializing` 状態とし、ロードバランサの選択対象外とする。  
...
9. エージェント登録完了後、対応モデル5件（gpt-oss:120b/20b、gpt-oss-safeguard:20b、qwen3-coder:30b、glm4:9b-chat）が当該エージェントで自動ダウンロード・常駐状態になること（ダッシュボードまたは API で確認可能であること）
