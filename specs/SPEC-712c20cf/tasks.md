...
## 追加要件（2025-11-01更新）
- 同一マシン上で複数のエージェントを起動する場合でも、`OLLAMA_PORT` が異なれば別インスタンスとして自動登録できること。
- OpenAI互換API (`/api/chat`, `/api/generate`) のハッピーパス/エラーケースを自動テスト（TDD）で保証すること。
- エージェント起動時にデフォルトモデル `gpt-oss:20b` を自動プルし、必要に応じて環境変数 `OLLAMA_DEFAULT_MODEL` で上書きできること。
- ダッシュボードのエージェント一覧でロード済みモデル列を表示し、詳細モーダルでも全モデルを確認できること。
- ダッシュボードのエージェント一覧テーブルはヘッダーと行の列幅が一致し、視覚的なずれが発生しないこと。
...
- [x] **T610** `GET /api/dashboard/metrics/:agent_id` を実装し、エージェントごとのCPU/メモリ履歴を返却
- [x] **T611** エージェント詳細モーダルにCPU/メモリ折れ線グラフを追加し、最新指標とエラーハンドリングを表示

## 本日のToDo (2025-10-31)

- [x] **T601** `coordinator/src/api/mod.rs` のルーター結線スモークテストを修正し、静的配信・API経路の確認を通す
- [x] **T602** `cargo test -p ollama-coordinator-coordinator` でダッシュボードAPI関連テストを実行し、失敗時は原因を特定して修正
...
- **詳細**:
  - エージェント（またはモック）を用意し、API経由でリクエストを送信する統合テストを追加
