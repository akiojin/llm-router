...
async fn test_audit_excludes_websocket() {
    ...
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM audit_log_entries WHERE request_path LIKE '/ws/%'")
            .fetch_one(&pool)
            .await
            .unwrap();
...
fn test_extract_actor_info_jwt_user() {
    ...
    let (actor_type, actor_id, _actor_username, api_key_owner_id) = extract_actor_info(&response);
...
async fn test_audit_captures_x_forwarded_for_ip() {
    ...
    let row: (Option<String>,) =
        sqlx::query_as("SELECT client_ip FROM audit_log_entries WHERE request_path = '/api/test'")
            .fetch_one(&pool)
            .await
            .unwrap();
