...
impl RequestHistoryStorage {
    ...
    /// レコードを保存
    pub async fn save_record(&self, record: &RequestResponseRecord) -> CoordinatorResult<()> {
    ...
    /// すべてのレコードを読み込み
    pub async fn load_records(&self) -> CoordinatorResult<Vec<RequestResponseRecord>> {
        let _guard = self.lock.lock().await;
        self.load_records_unlocked().await
    }

    /// 7日より古いレコードを削除
    pub async fn cleanup_old_records(&self, max_age: Duration) -> CoordinatorResult<()> {
        let _guard = self.lock.lock().await;
    ...
    pub async fn filter_and_paginate(
        ...
        page: usize,
        per_page: usize,
    ) -> CoordinatorResult<FilteredRecords> {
    ...
    async fn load_records_unlocked(&self) -> CoordinatorResult<Vec<RequestResponseRecord>> {
        // ファイルが存在しない場合は空配列
    ...
    async fn save_records_unlocked(&self, records: &[RequestResponseRecord]) -> CoordinatorResult<()> {
        ...
        if let Some(parent) = self.file_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                CoordinatorError::Database(format!("Failed to create directory: {}", e))
            })?;
        }

        // JSONに変換
        let json = serde_json::to_string_pretty(records).map_err(|e| {
            CoordinatorError::Database(format!("Failed to serialize records: {}", e))
        })?;
...
/// 履歴ファイルのパスを取得
fn get_history_file_path() -> CoordinatorResult<PathBuf> {
...
mod tests {
    ...
    use ollama_coordinator_common::protocol::RequestType;
    use std::net::IpAddr;
    ...
    async fn test_save_and_load_record() {
        ...
        std::env::set_var("OLLAMA_COORDINATOR_DATA_DIR", temp_dir.path());
