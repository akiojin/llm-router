...
use ollama_coordinator_common::auth::{ApiKey, ApiKeyWithPlaintext};
...
pub async fn create(
...
) -> Result<ApiKeyWithPlaintext, CoordinatorError> {
    let id = Uuid::new_v4();
    ...
    .execute(pool)
    .await
    .map_err(|e| CoordinatorError::Database(format!("Failed to create API key: {}", e)))?;
...
pub async fn find_by_hash(
    pool: &SqlitePool,
    key_hash: &str,
) -> Result<Option<ApiKey>, CoordinatorError> {
    ...
    .fetch_optional(pool)
    .await
    .map_err(|e| CoordinatorError::Database(format!("Failed to find API key: {}", e)))?;
...
pub async fn list(pool: &SqlitePool) -> Result<Vec<ApiKey>, CoordinatorError> {
    let rows = sqlx::query_as::<_, ApiKeyRow>(
        "SELECT id, key_hash, name, created_by, created_at, expires_at FROM api_keys ORDER BY created_at DESC"
    )
    .fetch_all(pool)
    .await
    .map_err(|e| CoordinatorError::Database(format!("Failed to list API keys: {}", e)))?;
...
pub async fn delete(pool: &SqlitePool, id: Uuid) -> Result<(), CoordinatorError> {
    sqlx::query("DELETE FROM api_keys WHERE id = ?")
        ...
        .execute(pool)
        .await
        .map_err(|e| CoordinatorError::Database(format!("Failed to delete API key: {}", e)))?;
